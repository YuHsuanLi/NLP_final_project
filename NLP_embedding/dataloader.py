...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, dict, key_lst, mode='w2v', one_reply=False, is_dev=False, device = None):
        self.dict = dict
        ...
        self.embedding_r_list = []
        with torch.no_grad():
            for index in range(len(self.key_lst)):
                idx = self.key_lst[index]
                if not self.is_dev:
                    self.labels.append(self.dict[idx]['label'])               
                article = ' '.join(self.dict[idx]['article'])
                input_ids = torch.tensor(self.tokenizer.encode(article)).unsqueeze(0).to(self.device)
                embedding_a = self.embedding(input_ids)[0][0][0]
                self.embedding_a_list.append(embedding_a)
                replys = []
                embedding_r_list_ = []
                for s in self.dict[idx]['reply']:
                    replys.append(' '.join(s['text']))     
                for reply in replys:
                    input_ids = torch.tensor(self.tokenizer.encode(reply)).unsqueeze(0).to(self.device)
                    embedding_r_list_.append(self.embedding(input_ids)[0][0][0])
                embedding_r_stack = torch.stack(embedding_r_list_)
                embedding_r = torch.mean(embedding_r_stack, 0)
                self.embedding_r_list.append(embedding_r)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'bert':
            ...
            for reply in replys:
                input_ids = torch.tensor(self.tokenizer.encode(reply)).unsqueeze(0).to(self.device)
                embedding_r_list.append(self.embedding(input_ids)[0][0][0])
            

            embedding_r_stack = torch.stack(embedding_r_list)
            embedding_r = torch.mean(embedding_r_stack, 0)
            '''
            embedding_a = self.embedding_a_list[index]
            embedding_r = self.embedding_r_list[index]
            #tokens_tensor = torch.cat((embedding_a, embedding_r))
            tokens_tensor = embedding_a
            if not self.is_dev:
